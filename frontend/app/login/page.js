...
export default function ProductsPage() {
  ...
  return (
    <div className="min-h-screen bg-cyber-bg font-sans text-cyber-text selection:bg-cyber-purple/30 pt-20 relative">
      
      {/* GLOBAL NAVBAR */}
      <nav className="fixed top-0 left-0 right-0 z-40 border-b border-cyber-border bg-slate-950/70 backdrop-blur-md">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between">
            ...
            <div className="flex items-center gap-5">
              ...
              <Link href="/profile" className="text-xs font-bold uppercase tracking-wider text-cyber-muted hover:text-cyber-cyan transition">👤 Profile </Link>
              <Link href="/cart" className="rounded-xl bg-purple-600/10 border border-purple-500/20 px-3.5 py-2 text-xs font-bold text-cyber-purple hover:bg-cyber-purple hover:text-white transition">🛒 View Cart</Link>
            </div>
          </div>
        </div>
      </nav>
      ...
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 relative z-10">
        <div className="mb-10 border-b border-cyber-border pb-5">
          <h1 className="text-2xl font-black tracking-tight bg-gradient-to-r from-white via-slate-200 to-cyber-purple bg-clip-text text-transparent sm:text-3xl">Discover Products</h1>
        </div>
        ...
        <div className="mb-12 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="relative w-full lg:max-w-md group">
            ...
            <input
              ...
              className="relative w-full rounded-xl border border-cyber-border bg-cyber-panel/60 backdrop-blur-sm px-5 py-3.5 pl-11 text-sm text-white focus:border-cyber-purple focus:outline-none"
            />
              ...
              <button
                ...
                className={`rounded-xl px-5 py-2.5 text-xs font-bold uppercase tracking-wider transition-all duration-200 ${
                  selectedCategory === category
                    ...
                    : "text-cyber-muted hover:text-slate-200 hover:bg-slate-800/40"
                }`}
              >
                {category === "all" 
                  ? "🛍️ All Products" 
        ...
        {filteredProducts.length === 0 ? (
          <div className="text-center py-24 bg-cyber-panel/20 rounded-2xl border border-cyber-border backdrop-blur-sm">
            <p className="text-cyber-muted font-medium font-mono text-sm">No inventory matched current query matrices.</p>
          </div>
        ) : (
                ...
                <div className="mt-5 flex flex-1 flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest text-cyber-purple font-mono">
                      ...
                      <span className="bg-slate-950/80 px-2 py-0.5 rounded-md border border-cyber-border text-cyber-muted">{product.category}</span>
                    </div>
                    ...
                    {product.details && product.details.length > 0 && (
                      <p className="text-[10px] text-cyber-muted line-clamp-3 mt-1.5 font-mono">⚙️ {product.details.join(" | ")}</p>
                    )}
                  ...
                  <div className="mt-6 pt-4 border-t border-cyber-border flex items-center justify-between">
                    ...
                    <button onClick={() => handleAddToCart(product)} className="rounded-xl bg-purple-500/10 text-cyber-purple border border-purple-500/20 px-4 py-2 text-xs font-bold hover:bg-purple-600 hover:text-white transition">
                      Add to Cart 🛒
                    </button>
                  </div>
                </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fadeIn">
          <div className="relative w-full max-w-sm overflow-hidden rounded-2xl border border-cyber-border bg-cyber-panel p-6 shadow-2xl transition-all">
            <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-purple-500 via-indigo-500 to-cyan-500" />
            <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-purple-500/10 text-cyber-purple border border-purple-500/20 shadow-xl mb-4">🛒</div>
            <div className="text-center">
              <h3 className="text-base font-bold text-cyber-text">Item Registered Successfully</h3>
              <p className="mt-2 text-xs text-cyber-muted line-clamp-2">
                <span className="text-cyber-purple font-semibold">{activeProduct?.name}</span> has been successfully added to your shopping cart.
              </p>
            </div>
            <div className="mt-6 flex flex-col gap-2">
              ...
              <button onClick={() => setIsModalOpen(false)} className="w-full rounded-xl bg-slate-950 border border-cyber-border py-3 text-xs font-bold uppercase tracking-wider text-cyber-muted hover:text-slate-200 transition">Continue Shopping 🛍️</button>
            </div>
          </div>
        </div>
      )}
