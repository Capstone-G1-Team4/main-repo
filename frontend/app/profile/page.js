 ...
 *  page displays, with defensive field-name fallbacks. */
function normalizeUser(raw) {
  const roleMap = { admin: "Admin", customer: "Customer", user: "Customer" };
  return {
    username: raw.username || raw.user_name || raw.email,
    fullName: raw.full_name || raw.fullName || raw.name || raw.email || "User",
    role: roleMap[raw.role] || raw.role || raw.user_role || "Customer",
    email: raw.email || "",
    ...
    async function loadProfileData() {
      ...
      try {
        ...
        const orderList = Array.isArray(ordersResponse)
          ? ordersResponse
          : ordersResponse?.items || ordersResponse?.orders || [];
