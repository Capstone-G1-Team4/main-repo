...
export default function HomePage() {
  ...
  return (
    <div className={`${cyberPageShell} pt-20`}>
      ...
      <div className={`${cyberHeroSection} hero-particles relative overflow-hidden`}>
        <div className={cyberHeroContainer}>
          <span className={`${cyberEyebrowBadge} animate-fade-in-up`}>
            <span className={cyberEyebrowDot} />
            Empowered by RAG &amp; Semantic Grounding Layers
          </span>

          <h1 className={`${cyberHeroHeading} animate-fade-in-up delay-200`}>
            NextGen Smart{" "}
            <span className={`${cyberHeroHighlight} animate-gradient-text`}>Marketplace</span>
          </h1>

          <p className={`${cyberHeroSubtext} animate-fade-in-up delay-400`}>
            Chat with our AI shopping assistant or browse the catalog directly.
          </p>

          <div className="mt-10 flex flex-wrap justify-center gap-4 animate-fade-in-up delay-500">
            <Link href="/chat" className={`${cyberButtonPrimary} animate-glow-pulse`}>
      ...
      <section className={cyberSectionContainer}>
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {PLATFORM_FEATURES.map((feature, i) => (
            <AnimateOnScroll key={feature.title} animation="scale" delay={i * 100}>
              <div className={`${cyberFeatureCard} card-glow`}>
                <div className={`${cyberFeatureIcon} animate-float`} style={{ animationDelay: `${i * 200}ms` }}>{feature.icon}</div>
      ...
      <section className={cyberSectionContainer}>
        ...
        <div className="grid grid-cols-2 gap-5 sm:grid-cols-3 lg:grid-cols-6">
          {categoryCards.map((cat, i) => (
            <AnimateOnScroll key={cat.category} animation="scale" delay={i * 80}>
              <button
                type="button"
                onClick={() => router.push(`/products?cat=${cat.category}`)}
                className={`${cyberCategoryCard} card-glow w-full`}
              >
                <span className={`${cyberCategoryIcon} animate-float`} style={{ animationDelay: `${i * 150}ms` }}>{cat.icon}</span>
        ...
        <div className="flex flex-col items-stretch gap-6 sm:flex-row sm:items-center">
          {HOW_IT_WORKS_STEPS.map((step, index) => (
            <div key={step.title} className="flex flex-1 flex-col items-center gap-6 sm:flex-row">
              <AnimateOnScroll animation="up" delay={index * 150}>
                <div className={`${cyberTimelineStep} card-glow w-full`}>
                  <span className={`${cyberTimelineStepNumber} animate-glow-pulse`}>{index + 1}</span>
                  <span className={`${cyberTimelineStepIcon} animate-float`} style={{ animationDelay: `${index * 200}ms` }}>{step.icon}</span>
                  <h3 className={cyberTimelineStepTitle}>{step.title}</h3>
                </div>
              </AnimateOnScroll>
              {index < HOW_IT_WORKS_STEPS.length - 1 && (
                <div className={`${cyberTimelineConnector} animate-fade-in delay-${(index + 1) * 200}`}>→</div>
              )}
            </div>
          ))}
        </div>
      ...
      <section className={cyberSectionContainer}>
        ...
        <div className="flex flex-wrap justify-center gap-3">
          {AI_TECHNOLOGIES.map((tech, i) => (
            <AnimateOnScroll key={tech.label} animation="scale" delay={i * 60}>
              <span className={`${cyberTechBadge} glass-hover`}>
                <span className={`${cyberTechBadgeIcon} animate-float`} style={{ animationDelay: `${i * 100}ms` }}>{tech.icon}</span>
                {tech.label}
              </span>
            </AnimateOnScroll>
          ))}
      ...
      <section className={cyberSectionContainer}>
        <AnimateOnScroll animation="scale">
          <div className={`${cyberStatsSection} animate-glow-pulse`}>
            <div className="grid grid-cols-2 gap-5 sm:grid-cols-4">
              {PLATFORM_STATS.map((stat, i) => (
                <AnimateOnScroll key={stat.label} animation="scale" delay={i * 100}>
                  <div className={`${cyberStatCard} card-glow`}>
                    <div className={`${cyberStatsIcon} animate-float`} style={{ animationDelay: `${i * 200}ms` }}>{stat.icon}</div>
      ...
      <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="mb-12 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between border-b border-cyber-border pb-10">
          <AnimateOnScroll animation="left" className="w-full lg:max-w-md">
        ...
        {filteredProducts.length === 0 ? (
          <div className={`${cyberEmptyState} animate-fade-in`}>
            <p className={cyberEmptyStateText}>No featured products matched your search.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
            {filteredProducts.map((product, index) => (
              <AnimateOnScroll key={`${product.brand}-${product.name}-${index}`} animation="scale" delay={index * 80}>
                <div className={`${cyberCard} card-glow`}>
      ...
      <section className={`${cyberSectionContainer} pb-24`}>
        <AnimateOnScroll animation="scale">
          <div className={`${cyberCTASection} animate-glow-pulse`}>
            <h2 className={`${cyberCTAHeading} animate-gradient-text`}>Ready to discover your next product?</h2>
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-4">
              <Link href="/chat" className={`${cyberButtonPrimary} glass-hover`}>
                💬 Start Chatting
              </Link>
              <Link href="/products" className={`${cyberButtonOutline} glass-hover`}>
                🛍️ Browse Catalog
              </Link>
            </div>
          </div>
